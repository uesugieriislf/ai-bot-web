<template>
  <div class="home">
    <div class="page" :style="{ paddingBottom: isMobile ? '165px' : '' }" :class="{ 'page-change': list.length }">
      ...
      <div v-show="list.length" id="myList" ref="contentListRef"
        ...
        <div class="chatList">
          <div v-show="item.text" v-for="(item, index) in list"
            :class="item.currentType === 'user' ? 'listItemP' : 'listItemA'" :key="index">
            ...
            ">
              ...
              <img class="listImg" src="/logo1.png" alt="" v-show="item.currentType === 'bot'" />

              <div v-if="item.currentType === 'bot'" class="botListText" ref="botListRefs">
                <div class="markdown-body" v-html="item.text"></div>
              </div>

              <div v-else class="listText">{{ item.text }}</div>
            </div>
            <div class="copyPcBtn" v-show="item.currentType === 'bot' && !isMobile">
      ...
      <div v-show="!list.length" class="content-box cancel-style">
        ...
        <div v-show="!isMobile" class="pList">
          <p v-for="item in pList" @click="handleHomeQuestion(item.qid)" :key="item">
            {{ item.question }}
          </p>
        </div>
      </div>
      <div class="bigBox">
        <div class="inputbox1" :style="{ maxWidth: isMobile ? '1000px' : '672px' }">
          <el-input v-bind:readonly="loading" @keypress="handleEnter" v-model="question" clearable type="textarea"
            :autosize="{ minRows: 1, maxRows: 8 }" id="message" placeholder="输入你的问题">
          </el-input>
          <el-button type="primary" size="small" v-if="!iscancel" @click="send">发送</el-button>
          <el-button v-else type="danger" size="small" @click="handleCancel">停止</el-button>
        </div>
        <div class="message">
          <p>
...
const conversationId = ref(""); //上下文id
...
async function send() {
  ...
  try {
    const obj = { message };
    let oldConversationId = localStorage.getItem('conversationId')
    if (oldConversationId) {
      conversationId.value = oldConversationId;
    }
    obj.conversationId = conversationId.value;
    sendInfo({
      data: obj,
...
function downloadPro(progressEvent) {
  ...
  // 如果没有conversationId，表明是新对话，把对话中的会话id赋值
  if (!conversationId.value) {
    conversationId.value = parts[1];
